// Package bot provides the Telegram bot interface for mux.
package bot
...
// Send sends a text message to the user, handling rate limiting.
func (b *Bot) Send(chatID int64, text string) error {
	...
	return b.enqueueSend(msg, PriorityNormal)
}

// SendWithPrefix sends a message with agent emoji+name prefix, auto-splitting.
func (b *Bot) SendWithPrefix(chatID int64, emoji, agentName, text string) error {
	if text == "" {
		return nil
	}
	...
	if len(parts) == 1 {
		full := fmt.Sprintf("%s %s: %s", emoji, agentName, parts[0])
	...
	for i, part := range parts {
		header := fmt.Sprintf("%s %s (%d/%d): ", emoji, agentName, i+1, len(parts))
		full := header + part
		msg := tgbotapi.NewMessage(chatID, full)
		msg.ParseMode = "Markdown"
